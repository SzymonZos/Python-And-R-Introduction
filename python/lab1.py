# Task 1
def find_position(string: str, key: str):
    return [position for position, char in enumerate(string) if char == key]


def find_positions(string: str, *keys):
    positions = {}
    for key in keys:
...
# Task 2
def positions_to_tuple_list(positions: dict):
    tuple_list = []
...
def positions_to_tuple_list_comprehension(positions: dict):
...
# Task 4
def draw_histo(input_dict: dict):
    new_dict = {key: len(input_dict[key]) for key in input_dict}
    for key, value in dict_sorted_generator(new_dict):
        print(key, "*" * value)


# Task 5
def convert_to_unique(input_list: list):
    return [*set(input_list)]


# Task 6
def jaccard_index(set_1: set, set_2: set):
    return len(set_1 & set_2) / len(set_1 | set_2)


# Task 7
def evaluate_sets(set_1: set, set_2: set):
    return (set_1, set_2) if len(set_1) >= len(set_2) else (set_2, set_1)


def remove_from_bigger_set(bigger: set, smaller: set):
    return bigger.remove((bigger - smaller).pop())


def optimize_jaccard_index(set_1: set, set_2: set, limit=0.9):
...
def main():
    ...
    # Task 7
    print(optimize_jaccard_index(s1, s2, 2))
